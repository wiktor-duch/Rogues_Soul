...
class Map:
            ...
            for room in self.rooms:
                if room.intersect_tile_at(x, y):
                    ...
                    if ((x == room.x1 and y == room.y1) or (x == room.x1 and y == room.y2) or
                        ...
                        if room.intersect_tile_at(x-1, y):
                            x -= 1
    ...
    def create_horiz_tunnel(self, x1: int, x2: int, y: int) -> None:
        '''
            ...
            for room in self.rooms:
                if room.intersect_tile_at(x, y):
                    ...
                    if ((x == room.x1 and y == room.y1) or (x == room.x1 and y == room.y2) or
                        ...
                        if room.intersect_tile_at(x, y-1):
                            y -= 1
    ...
    def generate_dungeon(
        ...
        for i in range(1, len(self.rooms)):
            ...
            (curr_x, curr_y) = self.rooms[i].center()
            ...
            if randint(0,1) == 1:
                ...
                self.create_horiz_tunnel(prev_x, curr_x, prev_y)
                self.create_vert_tunnel(prev_y, curr_y, curr_x)
            else:
                self.create_vert_tunnel(prev_y, curr_y, prev_x)
                self.create_horiz_tunnel(prev_x, curr_x, curr_y)